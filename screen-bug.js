...
<html>
  ...
  <body>
  <p style="text-align: left; background-color:#000000;padding:10px;color:#FFFFFF">Titre PDF - ${formatDate(new Date())}</p>
  <h3 style="text-align:center;padding-bottom:20px;border-bottom:1px solid #000000">
  <img src="source.jpg" width="300" alt="logo" border="0"></h3>
  ...
  data.forEach((item, i) => {
    ...
    item.parts.map((sub, j) => {
      ...
      sub.subParts.map((subItem, k) => {
        ...
        if (subItem.champsInput1) {
          string += `<h3>${subItem.champsInput1}:${fourni1}</h3>`;
        }
        if (subItem.champsInput2) {
          string += `<h3>${subItem.champsInput12}:${fourni2}</h3>`;
        }
...
const initialData = [
  {
    ...
    parts: [
      ...
      {
        title: "1 - Numéro de promo",

        subParts: [
          {
            champsInput1: "Inscrire le numéro",
            values: [
      ...
      },
      {
        ...
        subParts: [
          {
            title: "- Horaires à respecter 7h-11h",
            values: [
              { label: "Conforme", color: "green", value: false },
              { label: "Non conforme", color: "red", value: false },
            ],
          },
        ],
      },
...

      {
        ...
        subParts: [
          {
            title: "Emballages livrés conformes aux quantités commandées",
            values: [
              { label: "Conforme", color: "green", value: false },
              { label: "Non conforme", color: "red", value: false },
            ],
      ...
      {
        title: "4 - Températures",
        subParts: [
          {
            title: "Noter la temperature",
            champsInput2: "Température",
            values: [
...
const Envoi = () => {
  ...
  const onPrint = async () => {
    ...
    const suffix = `${formatDate(new Date())}-Titre-PDF`;
  ...
  const findNegativeComment = () => {
    ...
    if (comments2.length > 0) {
      Linking.openURL(
        `mailto:exemple@mail.com?subject=Alerte&body=${comments}+:\n\n`
      );
    }
  };
  ...
  return (
    ...
    <Block style={{ paddingHorizontal: 0 }} flex>
      ...
      <Button onPress={onFormClear}style={styles.buttonclear
        ...
        <Text>Effacer le formulaire</Text>
  </Button>
